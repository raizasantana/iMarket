...
<title>iMarket - Listar Sessões</title>
</head>
<body>
<!--
Barra Superior
...
<table border="2" >
   ...
   <c:forEach var="sessaos" items="${sessaos}">
   		<tr height="25">
   			<td>${sessaos.horaIni} </td> 
   			<td>${sessaos.data} </td> 
   			<td>${sessaos.filme.nome }</td>
   			<td>${sessaos.sala.nome}</td>
   			<td><a class="botao" href="/ProjetoFinal/Servlet?idsessao=${sessaos.id}&classe=IngressoControl&acao=lancar&destino=/html/frmingresso.jsp">Escolher Sessao</a></td>
